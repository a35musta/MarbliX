...
def load_embeddings(csv_path, image_dir, seq_dir):
    """
    Load image and sequence embeddings based on file names in a CSV file.

    Args:
        csv_path (str): Path to CSV file with 'file_name' and 'label' columns.
...
def main():
    ...
    # Load data
    image_emb, seq_emb, labels = load_embeddings(csv_path, image_path, seq_path)
    ...
    # Extract features
    real_features = extract_features(Model(inputs=[model.input[0], model.input[3]], outputs=model.layers[-2].output),
                                     encoded_img_test, encoded_seq_test)
    binary_features = extract_features(Model(inputs=[model.input[0], model.input[3]], outputs=model.layers[-1].output),
                                       encoded_img_test, encoded_seq_test, binarize=True)

    binary_matrices = np.reshape(binary_features, (-1, 8, 8))
